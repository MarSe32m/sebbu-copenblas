...
if not exist "%VSWHERE%" (
    echo ERROR: vswhere.exe was not found. Install Visual Studio with C++ ARM64 tools.
    goto :failed
)
...
for %%I in ("%ARTIFACT_BUNDLE_DIR%\openblas-windows-aarch64") do set "OUT_DIR=%%~fI"
...
echo.
echo ERROR: OpenBLAS ARM64 build or packaging failed with exit code %BUILD_EXIT_CODE%.
